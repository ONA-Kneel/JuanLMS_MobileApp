// StudentChats.js - Mobile version matching web Student_Chats.jsx
...
export default function StudentChats() {
  ...
  useEffect(() => {
    const fetchUserGroups = async () => {
      ...
      try {
        const token = await AsyncStorage.getItem('jwtToken');
        console.log('Fetching user groups from:', `${API_BASE}/group-chats/user/${user._id}`);
  ...
  const filteredUsers = users.filter(u => 
    u._id !== user?._id && 
    ['faculty', 'admin', 'principal', 'vpe'].includes(u.role?.toLowerCase()) &&
