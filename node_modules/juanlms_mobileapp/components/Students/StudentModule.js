...
export default function StudentModule(){
    ...
    const initializeSocket = async () => {
        try {
            ...
            console.log('[StudentModule] Socket initialized successfully, joining class room...');
    ...
    const handleNewAnnouncement = (data) => {
        try {
            ...
            if (socketClassID === classID) {
                ...
                // IMMEDIATE UI UPDATE - Force React Native to re-render
                setAnnouncements(prevAnnouncements => {
                    console.log('[StudentModule] BEFORE UPDATE - Previous announcements count:', prevAnnouncements.length);
                    
                    // Check if announcement already exists to prevent duplicates
                    const exists = prevAnnouncements.some(existing => 
                        existing._id === announcement._id || 
                        (existing.title === announcement.title && existing.message === announcement.message)
                    );
                    
                    if (exists) {
                        console.log('[StudentModule] Announcement already exists, skipping duplicate');
                        return prevAnnouncements;
                    }
                    
                    // Add new announcement to the top of the list
                    const updatedAnnouncements = [announcement, ...prevAnnouncements];
                    console.log('[StudentModule] AFTER UPDATE - Updated announcements count:', updatedAnnouncements.length);
                    console.log('🎉 [StudentModule] ANNOUNCEMENT SUCCESSFULLY ADDED TO UI STATE! 🎉');
                    console.log('[StudentModule] New announcement title:', announcement.title);
                    
                    return updatedAnnouncements;
                });

                // IMMEDIATE VISUAL FEEDBACK - Show alert banner
                console.log('[StudentModule] Setting visual alert for announcement:', announcement.title);
                setNewAnnouncementAlert(announcement.title);
                
                // Clear alert after 5 seconds
                setTimeout(() => {
                    console.log('[StudentModule] Clearing announcement alert');
                    setNewAnnouncementAlert(null);
                }, 5000);

                // BACKUP REFRESH - Ensure data consistency (but UI should already be updated)
                console.log('[StudentModule] Scheduling backup data refresh...');
                setTimeout(() => {
                    if (classID) {
                        console.log('[StudentModule] Performing backup refresh to ensure data consistency...');
                        fetchAnnouncements(classID); // Use fetchAnnouncements instead of fetchSpecificClass
                    }
