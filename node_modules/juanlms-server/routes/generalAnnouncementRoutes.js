...
// PUT /api/general-announcements/:id - Update an announcement
router.put("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (body && body.length > 2000) {
      return res.status(400).json({ message: "Body cannot exceed 2000 characters" });
    }

    // Update announcement
    const updatedAnnouncement = await GeneralAnnouncement.findByIdAndUpdate(
      announcementId,
      { title, body, recipientRoles, termName, schoolYear },
      { new: true }
    ).populate('createdBy', 'firstname lastname role');
...
// DELETE /api/general-announcements/:id - Delete an announcement
router.delete("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    await GeneralAnnouncement.findByIdAndDelete(announcementId);
