import { getLogger, OwnCapability } from '@stream-io/video-client';
...
export const usePermissionRequest = () => {
  ...
  const userHasUpdateCallPermissionsCapability = useHasPermissions(OwnCapability.UPDATE_CALL_PERMISSIONS);
  ...
  const handleUpdatePermission = useCallback((request, allow) => {
    return async () => {
      const {
        user,
        permissions
  ...
  useEffect(() => {
    ...
    return call.on('call.permission_request', event => {
      ...
      permissions.forEach(permission => {
        return Alert.alert('Permissions Request', messageForPermission(user.name ?? user.id, permission), [{
          text: 'Reject',
