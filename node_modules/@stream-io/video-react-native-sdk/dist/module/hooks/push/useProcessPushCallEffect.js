...
export const useProcessPushCallEffect = () => {
  ...
  useEffect(() => {
    ...
    getLogger(['useProcessPushCallEffect'])('debug', `Adding subscriptions to process incoming call from push notification`);
...
const createCallSubscription = (behaviourSubjectWithCallCid, client, pushConfig, action) => {
  return behaviourSubjectWithCallCid.pipe(distinctUntilChanged(), filter(cidIsNotUndefined)).subscribe(async callCId => {
    getLogger(['useProcessPushCallEffect'])('debug', `Processing call from push notification with action: ${action} and callCId: ${callCId}`);
