...
export const firebaseDataHandler = async data => {
  ...
  */
  ...
  if (data.type === 'call.ring') {
    ...
    if (asForegroundService) {
      ...
      notifee.registerForegroundService(() => {
        return new Promise(async () => {
          ...
          if (!client) {
            getLogger(['firebaseMessagingOnMessageHandler'])('debug', `Closing fg service as there is no client to create from push config`);
          ...
          if (_shouldCallBeClosed) {
            getLogger(['firebaseMessagingOnMessageHandler'])('debug', `Closing fg service callCid: ${call_cid} shouldCallBeClosed: ${_shouldCallBeClosed}`);
            notifee.stopForegroundService();
            return;
          }
          ...
          const unsubscribe = callFromPush.on('all', event => {
            ...
            if (!_canListenToWS) {
              getLogger(['firebaseMessagingOnMessageHandler'])('debug', `Closing fg service from event callCid: ${call_cid} canListenToWS: ${_canListenToWS}`, {
                event
              });
            ...
            if (_shouldCallBeClosed) {
              getLogger(['firebaseMessagingOnMessageHandler'])('debug', `Closing fg service from event callCid: ${call_cid} canListenToWS: ${_canListenToWS} shouldCallBeClosed: ${_shouldCallBeClosed}`, {
                event
              });
              unsubscribeFunctions.forEach(fn => fn());
              notifee.stopForegroundService();
          ...
          const subscription = callFromPush.state.callingState$.subscribe(callingState => {
            if (callingState === CallingState.IDLE || callingState === CallingState.LEFT) {
              getLogger(['firebaseMessagingOnMessageHandler'])('debug', `Closing fg service from callingState callCid: ${call_cid} callingState: ${callingState}`);
              unsubscribeFunctions.forEach(fn => fn());
              notifee.stopForegroundService();
            }
          });
    ...
    getLogger(['firebaseMessagingOnMessageHandler'])('debug', `Displaying incoming call notification with callCid: ${call_cid} title: ${title} body: ${body} asForegroundService: ${asForegroundService}`);
    ...
    if (shouldCallBeClosed(callFromPush)) {
      getLogger(['firebaseMessagingOnMessageHandler'])('debug', `Removing incoming call notification immediately with callCid: ${call_cid} as it should be closed`);
    ...
    getLogger(['firebaseMessagingOnMessageHandler'])('debug', `Displaying NonRingingPushEvent ${type} notification with title: ${title} body: ${body}`);
    await notifee.displayNotification({
...
export const onAndroidNotifeeEvent = async ({
  ...
  if (data.type === 'call.ring') {
    ...
    if (mustAccept || mustDecline || type === notifeeLib.EventType.ACTION_PRESS) {
      getLogger(['onAndroidNotifeeEvent'])('debug', `clearPushWSEventSubscriptions for callCId: ${call_cid} mustAccept: ${mustAccept} mustDecline: ${mustDecline}`);
      clearPushWSEventSubscriptions();
      notifee.stopForegroundService();
    }
    if (mustAccept) {
      getLogger(['onAndroidNotifeeEvent'])('debug', `pushAcceptedIncomingCallCId$ added with callCId: ${call_cid}`);
      pushAcceptedIncomingCallCId$.next(call_cid);
      // NOTE: accept will be handled by the app with rxjs observers as the app will go to foreground always
    } else if (mustDecline) {
      getLogger(['onAndroidNotifeeEvent'])('debug', `pushRejectedIncomingCallCId$ added with callCId: ${call_cid}`);
      ...
      if (hasObservers) {
        ...
        getLogger(['onAndroidNotifeeEvent'])('debug', `Skipped processCallFromPushInBackground for Declining call with callCId: ${call_cid} as the app is in the foreground state`);
        return;
      }
      getLogger(['onAndroidNotifeeEvent'])('debug', `start processCallFromPushInBackground - Declining call with callCId: ${call_cid}`);
      await processCallFromPushInBackground(pushConfig, call_cid, 'decline');
    } else {
      if (type === notifeeLib.EventType.PRESS) {
        getLogger(['onAndroidNotifeeEvent'])('debug', `pushTappedIncomingCallCId$ added with callCId: ${call_cid}`);
        ...
        getLogger(['onAndroidNotifeeEvent'])('debug', `pushAndroidBackgroundDeliveredIncomingCallCId$ added with callCId: ${call_cid}`);
    ...
    const notifeeLib = getNotifeeLibThrowIfNotInstalledForPush();
    if (type === notifeeLib.EventType.PRESS) {
      getLogger(['onAndroidNotifeeEvent'])('debug', `onTapNonRingingCallNotification with callCId: ${call_cid}`);
