...
export class StreamVideoRN {
  ...
  static onPushLogout() {
    if (pushLogoutCallbacks.current) {
      return Promise.all(pushLogoutCallbacks.current.map(callback => callback())).then(() => {});
    }
  ...
  static addOnNewCallNotificationListener(callback) {
    ...
    return () => {
      newNotificationCallbacks.current = newNotificationCallbacks.current?.filter(cb => cb !== callback);
    };
  }
}
