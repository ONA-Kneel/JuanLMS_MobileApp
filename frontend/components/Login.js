  ...
  useEffect(() => {
    ...
    const loadLockoutState = async () => {
      ...
      if (isRememberEnabled) {
        // Load credentials from secure storage (survives cache clearing)
        const credentialsResult = await SecureStorage.getCredentials();
  ...
  const persistRememberPreference = async (enabled) => {
    try {
      ...
      if (enabled && email && password) {
        // Save current credentials to secure storage (survives cache clearing)
  ...
  const saveCredentialsIfRemembered = async (currentEmail, currentPassword) => {
    try {
      if (rememberMe) {
        ...
        await SecureStorage.saveCredentials(currentEmail, currentPassword);
      } else {
