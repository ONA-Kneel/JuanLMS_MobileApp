...
export default function StudentMeeting() {
  ...
  useEffect(() => {
    if (activeTab === 'host-meeting' && allUsers.length === 0) {
      fetchAllUsers();
    }
  }, [activeTab]);
    ...
    try {
      ...
      if (classesData.status === 'fulfilled' && classesData.value) {
        ...
        if (activeClasses.length > 0) {
          ...
          // Fetch meetings for first class immediately, then fetch for others in parallel
          fetchMeetings(activeClasses[0]._id);
          // Optionally pre-fetch meetings for other classes in background
  ...
  const fetchAllUsers = async () => {
    try {
      ...
      // Only fetch users when needed (host meeting tab), not on initial load
